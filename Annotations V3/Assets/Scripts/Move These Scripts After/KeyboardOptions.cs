...
public class KeyboardOptions : MonoBehaviour
{
    ...
    private bool m_ToggledOn = false;
    private bool m_ToggledOff = true;

    [Header("Angles of which to turn on the menu")]
    [SerializeField] [Range(0.0f, 90.0f)] private float m_LessThanAngle = 40.0f;
    [SerializeField] [Range(0.0f, 90.0f)] private float m_GreaterThanAngle = 0.0f;
    ...
    [SerializeField] private VoidEvent m_MenuOpenEvent;
    [SerializeField] private VoidEvent m_MenuCloseEvent;

    [Header("Keyboard background")]
    [SerializeField] private KeyboardVisualScaler m_KeyboardBackgroundVisual;
    ...
    public void UpdateHorizontalKeySpace()
    {
        if (m_KeyboardActive.Value == true)
        {
            foreach (KeyboardKeySetup keySetup in m_KeySetup)
            {
                ...
                m_KeyboardBackgroundVisual.ScaleHorizontalVisuals((int)m_KeyHorizontalSpaceSlider.value);
            }
        }
    }
    ...
    public void UpdateVerticalKeySpace()
    {
        if (m_KeyboardActive.Value == true)
        {
            //Space the rows vertically
            m_KeyboardBackgroundVisual.UpdateVerticalSpacing((int)m_KeyVerticalSpaceSlider.value);
        }
    }
