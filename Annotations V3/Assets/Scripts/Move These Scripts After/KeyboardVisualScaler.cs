...
public class KeyboardVisualScaler : MonoBehaviour
{
    ...
    public float m_VerticalOffset = 0.22f;
    ...
    public void UpdateVerticalSpacing(int sliderValue)
    {
        ...
        Vector3 topPreviousPosition = m_CenterGameObject.transform.localPosition;
        Vector3 bottomPreviousPosition = m_CenterGameObject.transform.localPosition;

        //Add offset to top rows
        for (int i = 0; i < m_BottomRows.Length; i++)
        {
            Vector3 newPos = bottomPreviousPosition;
            ...
            bottomPreviousPosition = newPos;
        }


        //Add offset to right side
        for (int i = m_TopRows.Length - 1; i >= 0; i--)
        {
            Vector3 newPos = topPreviousPosition;

            //Only add half the offset for the first set of keys
            if (i == m_TopRows.Length - 1)
            {
                newPos.z += offset / 2;
            }
            else
